...
function showTooltip(event, d, d2) {
    ...
    d3.select("#tooltip-count").text(`Count: ${d.value}`);
}
...
function topFoods(d, d2) {
    ...
    d3.select("#p-hour-hour").text(`Popular Foods: Day: ${d.day} Hour: ${d.hour} `);
...
async function fetchCSVData() {
    const data1 = await fetchCSV('./data/data1.csv');
    const data2 = await fetchCSV_2('./data/avg_foods.csv');
    if (data1 && data2) {
